...
public class UserDAO {
    ...
    public User create(User user) throws SQLException {
        ...
        try {
            ...
            ps.setString(4, user.getUsuario()); // Tu campo para el Login
            ps.setString(5, PasswordHasher.hashPassword(user.getClave())); // Hashear la clave antes de guardarla
            ps.setInt(6, user.getIdRol()); // Id del Rol (Admin/Solicitante) de tus prototipos
            ps.setString(7, user.getFotoPerfil()); // Ruta de la foto elegida
            ps.setByte(8, user.getEstado()); // Tu variable byte (1 o 0)
    ...
    public boolean update(User user) throws SQLException {
        ...
        try {
            // Preparar la sentencia SQL adaptada a tu tabla y campos en español
            ps = conn.connect().prepareStatement(
                    "UPDATE Usuarios " +
            ...
            ps.setString(8, user.getUltimaModificacionPor()); // Guarda el Administrador que modificó
            ps.setInt(9, user.getIdUsuario()); // Condición para identificarlo por su IdUsuario
    ...
    public User authenticate(User user) throws SQLException {
        User userAuthenticate = new User(); // Objeto para almacenar el usuario logueado.
        ...
        try {
            ...
            ps.setString(1, user.getUsuario()); // Cambiado a getUsuario()
            ps.setString(2, PasswordHasher.hashPassword(user.getClave())); // Cambiado a getClave()
            rs = ps.executeQuery();
            ...
            if (rs.next()) {
                ...
                userAuthenticate = null; // Credenciales incorrectas o usuario inactivo
            }
            ps.close();
            rs.close();
        } catch (SQLException ex){
